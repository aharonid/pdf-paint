...
A tiny local paint app for PDFs. It rasterizes every page of a PDF into a bitmap, hands you
MS-Paint-style tools, and packs the edited bitmaps back into a PDF.

Fully offline. No build step, no npm install, no network calls at runtime — `pdf.js` and
`pdf-lib` are vendored in `vendor/`.
